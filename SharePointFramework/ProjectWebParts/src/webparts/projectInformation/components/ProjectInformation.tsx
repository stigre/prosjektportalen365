...
export default class ProjectInformation extends React.Component<IProjectInformationProps, IProjectInformationState> {
  ...
  public async componentDidMount() {
    ...
    const projectsList = hubSiteRootWeb.lists.getByTitle(this.props.listName);
    const fields = await hubSiteRootWeb.contentTypes.getById(this.props.contentTypeId).fields.filter(`Group eq '${this.props.fieldsGroup}'`).get();
    const spEntityPortalService = new SpEntityPortalService(hubSite.SiteUrl, this.props.listName, 'GtGroupId');
